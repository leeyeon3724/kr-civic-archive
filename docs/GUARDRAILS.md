...
- 정책/운영 변경 PR은 `docs/API.md`, `docs/ENV.md`, `docs/ARCHITECTURE.md`, `docs/OPERATIONS.md` 정합성을 함께 갱신

## 릴리스 문맥

- PR 문맥 통과 후 아래 게이트를 추가 실행
...
- 장애 모드 점검 시 readiness degraded를 명시적으로 허용

```bash
python scripts/check_runtime_health.py --base-url http://localhost:8000 --allow-ready-degraded
```

- 인시던트 종료 후 `docs/OPERATIONS.md` 템플릿 기준으로 회고를 기록하고 필요 시 `docs/CHANGELOG.md` `Unreleased`에 반영
